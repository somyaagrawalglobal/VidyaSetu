...
export const sendVerificationEmail = async (to, token) => {
  ...
  await transporter.sendMail({
    from: `VidyaSetu Verify<${process.env.EMAIL_FROM}>`,
    to,
    ...
    html: `
      <div style="background:#F9FAFB; padding:40px 10px; font-family: Arial, Helvetica, sans-serif;">
        <div style="max-width:520px; margin:0 auto; background:#ffffff; border-radius:14px; padding:40px 30px; text-align:center; border:1px solid #E5E7EB;">
          ...
          <!-- Title -->
          <h2 style="margin:0 0 12px; font-size:22px; color:#111827; line-height:1.4;">
            Please verify your email <span style="display:inline-block; font-size:24px; vertical-align:middle;">😃</span>
          </h2>
...
export const sendPasswordResetEmail = async (to, token) => {
  ...
  await transporter.sendMail({
    from: `VidyaSetu Support<${process.env.EMAIL_FROM}>`,
    to,
...
export const sendPurchaseEmail = async ({ to, userName, courseName, coursePrice, orderId, status, courseUrl }) => {
  ...
  await transporter.sendMail({
    from: `VidyaSetu Notification<${process.env.EMAIL_FROM}>`,
    to,
        ...
        <div style="${styles.container}">
          <div style="${styles.header}">
            ...
            <div style="${styles.iconBox}">
              <span style="font-size: 32px; line-height: 64px; display: inline-block;">📚</span>
            </div>
      ...
      <div style="${styles.wrapper}">
        <div style="${styles.container}">
          <div style="${styles.header}">
            <img src="${getAbsoluteUrl('assets/images/brand-logo.png')}" alt="VidyaSetu" style="${styles.logo}" width="120" />
            <div style="${styles.iconBox}">
              <span style="font-size: 32px; line-height: 64px; display: inline-block;">${status === 'approved' ? '✅' : '📝'}</span>
            </div>
...
export const sendContactEmail = async ({ fullName, email, phoneNumber, subject, message, otherTopic }) => {
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://vidyasetu.com';

  // Premium Layout Styles
  const styles = {
    wrapper: 'background-color: #f1f5f9; padding: 40px 10px; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif;',
    container: 'background-color: #ffffff; max-width: 600px; margin: 0 auto; border-radius: 20px; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.05), 0 8px 10px -6px rgba(0, 0, 0, 0.01); border: 1px solid #e2e8f0; overflow: hidden;',
    header: 'background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); padding: 40px 40px 32px; text-align: center; color: white;',
    logo: 'height: 48px; width: auto; margin-bottom: 24px;',
    headerTitle: 'font-size: 24px; font-weight: 700; margin: 0 0 8px; letter-spacing: -0.5px; color: #ffffff;',
    headerSubtitle: 'font-size: 15px; color: #94a3b8; margin: 0; font-weight: 500;',
    body: 'padding: 40px;',
    sectionTitle: 'color: #94a3b8; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 20px; display: block;',
    dataGroup: 'margin-bottom: 24px;',
    label: 'color: #64748b; font-size: 13px; font-weight: 600; margin-bottom: 6px; display: block;',
    value: 'color: #1e293b; font-size: 16px; font-weight: 500;',
    link: 'color: #4f46e5; text-decoration: none; font-weight: 500;',
    messageCard: 'background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; font-size: 15px; line-height: 1.6; color: #334155; margin-top: 8px; white-space: pre-wrap;',
    actionButton: 'display: inline-block; background: #4f46e5; color: #ffffff; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px; margin-top: 8px;',
    footer: 'background-color: #f8fafc; padding: 32px 40px; text-align: center; border-top: 1px solid #e2e8f0;',
    footerText: 'color: #94a3b8; font-size: 13px; margin: 0 0 8px; line-height: 1.5;',
    badge: 'display: inline-block; background: rgba(255, 255, 255, 0.1); border: 1px solid rgba(255, 255, 255, 0.2); padding: 4px 12px; border-radius: 9999px; font-size: 12px; font-weight: 600; color: #e2e8f0; margin-bottom: 16px;'
  };

  const topicDisplay = subject === 'other' ? `Other: ${otherTopic}` : subject.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());

  await transporter.sendMail({
    from: `VidyaSetu Support <${process.env.EMAIL_FROM}>`,
    to: process.env.EMAIL_SERVER_USER,
    replyTo: email,
    subject: `[New Inquiry] ${fullName} - ${topicDisplay}`,
    html: `
      <div style="${styles.wrapper}">
        <div style="${styles.container}">
          ...
          <!-- Header -->
          <div style="${styles.header}">
            <img src="${getAbsoluteUrl('assets/images/brand-logo.png')}" alt="VidyaSetu" style="${styles.logo}" width="140" />
            <div>
              <span style="${styles.badge}">New Contact Form Submission</span>
            </div>
            <h1 style="${styles.headerTitle}">${topicDisplay}</h1>
            <p style="${styles.headerSubtitle}">Received from ${fullName}</p>
          </div>
          ...
          <div style="${styles.body}">
            ...
            <div style="display: flex; flex-wrap: wrap; margin: 0 -12px;">
              ...
              <div style="flex: 1; min-width: 240px; padding: 0 12px;">
                <span style="${styles.sectionTitle}">Contact Details</span>
                ...
                <div style="${styles.dataGroup}">
                  ...
                  <div style="${styles.value}">${phoneNumber}</div>
                </div>
              </div>
            </div>

            <div style="margin-top: 32px;">
              <span style="${styles.sectionTitle}">Message Content</span>
            ...
            <div style="margin-top: 40px; text-align: center;">
              <a href="mailto:${email}" style="${styles.actionButton}">Reply to ${fullName.split(' ')[0]}</a>
            </div>
          ...
          <div style="${styles.footer}">
            <p style="${styles.footerText}">
              This email was properly authenticated and sent via the VidyaSetu platform.
            </p>
            <p style="${styles.footerText}">
              &copy; ${new Date().getFullYear()} VidyaSetu. All rights reserved.
            </p>
